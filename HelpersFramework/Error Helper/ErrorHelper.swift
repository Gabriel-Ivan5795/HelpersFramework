...
open class ErrorHelper: NSObject {
    ...
    open func createGenericErrorWith200() -> NSError {
        return NSError(domain: self.stringDomain, code: 500, userInfo: ["status": "Error", "error": "alert_an_error_has_occured_200"])
    }
    
    open func createGenericErrorWith500() -> NSError {
        return NSError(domain: self.stringDomain, code: 500, userInfo: ["status": "Error", "error": "alert_an_error_has_occured_500"])
    }
    
    open func createGenericErrorWithmy500() -> NSError {
        return NSError(domain: self.stringDomain, code: 500, userInfo: ["status": "Error", "error": "alert_an_error_has_occured_my_500"])
    }
    ...
    open func createNotFoundError() -> NSError {
        return NSError(domain: self.stringDomain, code: 404, userInfo: ["status": "Error", "error": "alert_categories_not_found"])
    }
